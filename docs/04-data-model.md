...
> **状态**：正式落地文档。字段命名、类型、枚举取值、索引、约束的唯一事实源为 `docs/_canonical-data-model.md`（权威数据模型基线）；行为契约（状态机迁移、路由、产能）以 `docs/_canonical-workflow.md` 为准；页面字段展示形态以 `docs/_canonical-ia.md` 为准；MVP 范围与落库排期以 `docs/_canonical-roadmap.md` 为准。本文档只做**整理与落地说明**，不新增、不改写任何表字段；与基线冲突时一律以基线为裁决依据。
>
> **技术基线**：PostgreSQL 15+（Supabase）、Drizzle ORM、TypeScript strict。所有主键 `uuid PK DEFAULT gen_random_uuid()`（除非另有说明），所有时间戳 `timestamptz DEFAULT now()`。
>
> **阅读指引**：§3 枚举字典 → §4 表定义（34 表，按 9 域）→ §5 ERD → §6 关系链路 → §7 索引与约束汇总 → §8 数据完整性规则 → §9 关键业务规则 → §10 落库顺序 → §11 Open Questions。
...
- 全部表定义：34 张表（`_canonical-data-model.md` §3，按域整理，含列级类型/约束/索引/决策说明）；
- mermaid ERD（引用基线 §4 原图）；
- 枚举字典：9 个域、全量枚举取值（基线 §1）；
- 索引与约束：每表索引 + 全局约束汇总（§7）；
- 数据完整性规则：类型/唯一/触发器/防环/rollup/幂等/人工门禁（§8）；
- 关键业务规则：`topic_id` / `batch_id` 生成、血缘建模、快照不可变、评分 → priority、单一主 CTA、衍生预算、AI Weekly 口径（§9）。

权威链路：`_requirements.md`（原始需求）→ `_canonical-data-model.md`（字段唯一裁决）→ `_canonical-workflow.md`（行为契约）→ `_canonical-ia.md`（页面结构）→ `_canonical-roadmap.md`（范围/排期/决策日志 DC/OQ）。
...
| 约定 | 规则 |
|---|---|
| 命名 | 表名、列名、枚举取值一律小写下划线（snake_case）；枚举在 §3 统一定义，表中用类型名引用（如 `topic_type`） |
| 枚举落库 | `CREATE TYPE` 或 `text + CHECK` 二选一，由 Drizzle schema 决定；**V1 推荐 `text + CHECK`**（便于迁移回滚，决策 DC-42） |
| 主键 | `id uuid PK DEFAULT gen_random_uuid()`，所有表通用，除 `image_type_priorities`（以 `image_type` text 为 PK，基线另有说明） |
| 时间 | 一律 `timestamptz`；纯日历日（无时区语义）用 `date`（如 `workflow_batches.week_start/week_end`、`content_metrics.metric_date`） |
| `jsonb` | 仅用于动态结构（评分依据 `score_rationale`、产物载荷 `content`、配置 `value`、证据结构 `key_numbers` 等）；**禁止在 jsonb 内嵌关键外键做业务联查主路径** |
| 审计时间 | 所有表含 `created_at`；多数含 `updated_at`（例外：`image_type_priorities`、`knowledge_concept_edges`、`knowledge_derivations`、`trend_radar`、`workflow_outputs`、`workflow_tasks`、`audit_log` 仅 `created_at`） |
| 全局约束 | 人工审核门禁（需求硬性原则 6）与执行留痕（硬性原则 5）为全局约束，见 §8.6 / §9.6 |
...
| `topic_status` | `Draft` / `Researching` / `Ready for Production` / `Producing` / `Review` / `Needs Revision` / `Ready to Publish` / `Published` / `Archived` | 全局 9 态状态机（迁移见 `_canonical-workflow.md` §7） |
| `priority` | `P0` / `P1` / `P2` / `P3` | 由五维评分按类型权重推导（§9.5） |
...
| `selection_status` | `pending` / `selected` / `eliminated` | 候选事件入选状态 |
...
| 枚举名 | 取值 | 说明 |
|---|---|---|
| `source_type` | `official` / `github` / `official_docs` / `authoritative_media` / `tech_media` / `community` / `internal` | 7 类来源类型 |
| `source_verification_status` | `unverified` / `partially_verified` / `verified` / `conflict` / `needs_update` | 全局核验五态；GitHub/AI Weekly 复用同一枚举，禁止另造 |
...
| 枚举名 | 取值 | 说明 |
|---|---|---|
| `workflow_type` | `orchestrator` / `ai_weekly` / `github_weekly` / `evergreen_knowledge` / `wechat_deep_dive` | 4 子 Workflow + Orchestrator |
| `workflow_run_status` | `queued` / `running` / `completed` / `failed` / `needs_review` | run 状态；`needs_review` 为人工门禁吸收态 |
| `workflow_task_status` | `queued` / `running` / `completed` / `failed` / `skipped` | 步骤级状态（比 run 多 `skipped`） |
...
| `knowledge_category` | `ai_technology` / `ai_product` / `methodology` / `industry_practice` / `tool_tutorial` | 知识分类（需求未枚举，基线追加建议） |
...
| 枚举名 | 取值 | 说明 |
|---|---|---|
| `content_role` | `traffic` / `cognition` / `scenario` / `product` / `conversion` | 写作前必选、单值；全系统唯一 Content Role 定义 |
| `deep_dive_plan_status` | `drafting` / `review` / `needs_revision` / `approved` / `archived` | 蓝图审核状态，衔接人工审核 |
| `image_type` | `real_product_screenshot` / `real_ui` / `structure_infographic` / `flow_diagram` / `data_chart` / `concept_diagram` / `decorative` | 7 类配图类型 |
...
| 枚举名 | 取值 | 说明 |
|---|---|---|
| `asset_type` | `ai_weekly_script` / `short_video_script` / `wechat_article` / `github_card` / `xiaohongshu` / `sales_material` / `infographic` / `cover` | 8 类 |
| `asset_status` | `Draft` / `Producing` / `Review` / `Needs Revision` / `Ready to Publish` / `Published` / `Archived` | 资产制作/审核状态 = `topic_status` 子集 |
| `brand_asset_type` | `logo` / `template` / `product_screenshot` / `background` / `cta_card` / `ui_screenshot` / `visual_reference` | 品牌素材类型 |
...
### 3.7 Publication / Metrics 域

| 枚举名 | 取值 | 说明 |
|---|---|---|
| `publication_platform` | `wechat` / `douyin` / `xiaohongshu` / `bilibili` / `wechat_video` | 统一平台枚举（5 个，全表共用） |
| `publication_status` | `planned` / `ready` / `published` / `failed` | 发布执行状态；`published` 仅人工回填 |
...
| `selection_basis` | `pure_weekly_rank` / `value_filtered` / `mixed` | 正式榜单口径 |
...
| 强度指标（非枚举） | `signal_strength` / `velocity` / `novelty_score` 均为 `smallint CHECK (1-10)` | 三信号分量 |
...
| `topic_id` | `text` | NOT NULL, UNIQUE | 业务 ID `2026W36-001`，规则见 §9.1 |
...
| `priority` | `priority` | NOT NULL | P0-P3，推导见 §9.5 |
...
| `source_packet_id` | `uuid` | NULL, FK→`source_packets.id` | 当前生效证据包指针（循环引用破环，见 §4.2） |
...
**说明**：Lineage 查询 = `WITH RECURSIVE` 从某 Topic 沿 `parent`/`source` 边上下遍历；`topics.parent_topic_id` 与 `topics.source_topic_ids` 是需求必填的投影列，由 `lineage service` 单事务同写（§9.3）。
...
|---|---|---|---|
| `id` | `uuid` | PK | |
| `key` | `text` | NOT NULL, UNIQUE | `book_demo` / `download_whitepaper` / `join_community` / `contact_sales` / `follow_account` / `signup_newsletter` 等（种子词表全集见 §11 OQ-DM-03） |
...
### 4.2 Domain B — Source 域（3 表，三层模型：注册 / 聚合 / 明细）
...
|---|---|---|---|
| `id` | `uuid` | PK | |
| `batch_id` | `text` | NOT NULL, UNIQUE | `2026W36-AI-WEEKLY`；同周重跑加 `-02` 后缀（§9.2） |
...
| `task_type` | `text` | NOT NULL | `fact_check` / `score` / `select` / `script_generate` / `outline_generate` / `trend_radar` / `secondary_candidates` / `return_writeback` / `snapshot_capture` / `image_plan` 等（权威取值见 `_canonical-workflow.md` §5） |
...
| `output_type` | `workflow_output_type` | NOT NULL | 11 类产物 |
...
**说明**：AI 原始产出一律先进此表（留痕），**仅人工审核通过后提升为 `content_assets`**（见 §4.7），两表边界 = "原始产出 vs 审核后资产"。
...
**说明**：concept 图（知识前置关系）与 `topics.parent_topic_id/source_topic_ids`（选题/内容衍生关系）是**两套独立边，不互写**（决策 DC-23）。
...
| `source_ref_id` | `uuid` | NULL | FK→`brand_assets.id` 或 `source_packet_items.id`（版权来源，见 §11 OQ-DM-04） |
...
### 4.7 Domain G — Content / Asset 域（4 表）
...
**索引**：`INDEX(topic_id)`；`INDEX(asset_type)`；`INDEX(status)`；`GIN(asset_key)`（见 §11 OQ-DM-01 [REVIEW]）。
...
### 4.8 Domain H — Publication / Metrics / Leads 域（4 表）
...
**索引**：`INDEX(topic_id)`；`INDEX(platform)`；`INDEX(metric_date)`；`INDEX(publication_id)`；`UNIQUE(topic_id, asset_id, platform, metric_date, COALESCE(publication_id::text, ''))`（PG15 可改用 `UNIQUE NULLS NOT DISTINCT`，实现二选一见 §11 OQ-DM-02）。
...
> 引用基线 `_canonical-data-model.md` §4 原图，保持权威一致，未做改动。
...
**读图要点**：`topics` 处于全图枢纽位置（与 assets、publications、metrics、event_pool、workflow_runs、ctas、clusters、source_packets、knowledge_topic_bank、deep_dive_plans、trend_radar、leads 全部相连）；`source_packets` 与 `topics` 为双向引用（`topics.source_packet_id` 可空破环，见 §4.2）；`workflow_runs` 自引用构成调用树；`github_snapshot_items` 自引用构成 Replay 血缘。
...
| **血缘** | `topic_relations` 是权威图存储，`topics.parent_topic_id`（单父树）+ `topics.source_topic_ids`（多对多数组）为需求必填投影；Lineage 查询经递归 CTE（深度 3-5 层） |
...
| **候选提升链路** | `event_pool`（候选/事件，携带需求五九字段 + 入选/淘汰）→ Orchestrator 查重聚类（`topic_clusters`）→ 入选提升为 `topics`（`derived_topic_id` 回填） |
...
| **快照** | `github_snapshots`（`UNIQUE(week, snapshot_type, selection_basis)` + frozen 触发器）→ `github_snapshot_items`（捕获列冻结/运营列可变；Replay 新行 + `source_item_id` 血缘） |
...
### 7.1 唯一约束（UNIQUE / 部分唯一）

| 表 | 唯一约束 | 语义 |
|---|---|---|
| `topics` | `UNIQUE(topic_id)` | 业务 ID 全局唯一 |
| `topic_relations` | `UNIQUE(from_topic_id, to_topic_id, relation_type)` | 同类型边唯一 |
| `event_pool` | `UNIQUE(candidate_id)` | 候选 ID 全局唯一 |
| `topic_clusters` | `UNIQUE(cluster_key, week)` | 同周聚类键唯一 |
| `ctas` | `UNIQUE(key)` | CTA 词表键唯一 |
| `sources` | `UNIQUE(source_url, source_type)` | 同类型下 URL 唯一 |
| `source_packets` | `UNIQUE(packet_id)` | 证据包 ID 唯一 |
| `workflow_types` | `UNIQUE(key)` | 工作流类型键唯一 |
| `workflow_templates` | `UNIQUE(workflow_type_key, version)` | 类型下版本唯一 |
| `workflow_batches` | `UNIQUE(batch_id)` | 批次 ID 唯一 |
| `workflow_runs` | `UNIQUE(run_number)` | run 编号唯一 |
| `workflow_tasks` | `UNIQUE(run_id, sequence)` | run 内步骤序号唯一 |
| `ai_prompt_templates` | `UNIQUE(key, version)` | Prompt 键下版本唯一 |
| `github_snapshots` | `UNIQUE(week, snapshot_type, selection_basis)` | 同口径同周仅一个快照（不可变保障 1） |
| `github_snapshot_items` | `UNIQUE(snapshot_id, rank)` | 快照内名次唯一 |
| `knowledge_topic_bank` | `UNIQUE(topic_id)` | 与 topics 1:1 |
| `knowledge_concept_edges` | `UNIQUE(source_concept_id, target_concept_id, edge_type)` | 同类型边唯一 |
| `knowledge_derivations` | `UNIQUE(workflow_run_id, round_index)` | 衍生预算 DB 兜底（≤3） |
| `content_assets` | `UNIQUE(asset_key)` | 逻辑资产标识唯一 |
| `content_asset_versions` | `UNIQUE(asset_id, version)` | 资产内版本唯一 |
| `brand_assets` | `PARTIAL UNIQUE (is_primary_logo) WHERE is_primary_logo = true` | 官方 Logo 全局唯一 |
| `content_metrics` | `UNIQUE(topic_id, asset_id, platform, metric_date, COALESCE(publication_id::text, ''))`（PG15 可改 `UNIQUE NULLS NOT DISTINCT`） | 同 Topic×资产×平台×日×发布的指标行唯一 |
| `system_settings` | `UNIQUE(key)` | 配置键唯一 |

### 7.2 CHECK 约束（范围/格式）

| 位置 | CHECK | 说明 |
|---|---|---|
| `topics.content_week` | `^[0-9]{4}W[0-9]{2}$` | ISO 周格式 |
| `topics`（五维） | `b2b_relevance` / `traffic_potential` / `conversion_potential` / `timeliness` / `content_value` ∈ 1-10 | 五维评分范围 |
| `topics.business_relevance` | 1-10 | 门控评分范围 |
| `sources.source_quality_score` | 1-5 | 来源质量分范围 |
| `topic_relations` | `from_topic_id <> to_topic_id` | 防自环 |
| `knowledge_topic_bank` | `b2b_relevance` / `long_term_value` ∈ 1-10 | 知识评分范围 |
| `knowledge_derivations` | `round_index` ∈ 1-3 | 衍生预算上限 |
| `deep_dive_image_plans.image_priority_rank` | 1-7 | 配图优先级范围 |
| `image_type_priorities.priority_rank` | 1-7 | 7 类配图排序 |
| `brand_assets` | `CHECK (type = 'logo' → ai_policy = 'reference_only')` | Logo 禁止 AI 重绘（数据层强制） |
| `trend_radar` | `signal_strength` / `velocity` / `novelty_score` ∈ 1-10 | 信号分量范围 |
| `content_metrics` | `completion_rate` / `five_second_retention` ∈ `numeric(5,4)`（0-1） | 率值精度 |
| `content_assets` | `asset_type='wechat_article'` 时 `content_role` 必填 | 微信文章角色必选（单值） |
| `deep_dive_plans.content_role` | 单值（单值列 + CHECK） | 只允许一个 Content Role |

### 7.3 触发器（Trigger）

| 表 | 触发器 | 行为 |
|---|---|---|
| `github_snapshots` | `BEFORE UPDATE OR DELETE` 且 `status='frozen'` | `RAISE EXCEPTION` 拒绝任何修改/删除（含 items 一并冻结；软删除策略：不提供物理删除，仅允许新增 Replay 行） |
| `publications` | 禁止 workflow 直写 `published` | DB 触发器或应用层权限：`published` 仅人工触发并回填 `published_date / published_url / published_by` |

### 7.4 普通索引（查询路径）

| 表 | 索引 |
|---|---|
| `topics` | `INDEX(status)`；`INDEX(priority)`；`INDEX(topic_type)`；`INDEX(content_week)`；`INDEX(parent_topic_id)`；`GIN(trend_tags)` |
| `topic_relations` | `INDEX(to_topic_id)`（反向遍历）；`INDEX(relation_type)` |
| `event_pool` | `INDEX(batch_id)`；`INDEX(week)`；`INDEX(selection_status)`；`INDEX(cluster_id)` |
| `source_packets` | `INDEX(topic_id)` |
| `source_packet_items` | `INDEX(packet_id)`；`INDEX(source_id)` |
| `workflow_runs` | `INDEX(status)`；`INDEX(batch_id)`；`INDEX(topic_id)`；`INDEX(parent_run_id)`；`INDEX(workflow_type_key)` |
| `workflow_tasks` | `INDEX(run_id)` |
| `workflow_outputs` | `INDEX(run_id)`；`INDEX(output_type)`；`INDEX(applied)` |
| `github_snapshot_items` | `INDEX(snapshot_id)`；`INDEX(topic_id)` |
| `knowledge_topic_bank` | `INDEX(knowledge_status)`；`INDEX(content_status)`；`GIN(upstream_concepts)` |
| `deep_dive_plans` | `INDEX(topic_id)`；`INDEX(status)` |
| `content_assets` | `INDEX(topic_id)`；`INDEX(asset_type)`；`INDEX(status)` |
| `content_asset_versions` | `INDEX(asset_id)`；`INDEX(created_by_run_id)` |
| `brand_assets` | `INDEX(type)`；`INDEX(active)` |
| `asset_brand_usages` | `INDEX(content_asset_id)`；`INDEX(brand_asset_id)` |
| `publications` | `INDEX(topic_id)`；`INDEX(status)`；`INDEX(platform)`；`INDEX(asset_id)` |
| `content_metrics` | `INDEX(topic_id)`；`INDEX(platform)`；`INDEX(metric_date)`；`INDEX(publication_id)` |
| `trend_radar` | `INDEX(radar_week)`；`INDEX(topic_id)` |
| `leads` | `INDEX(topic_id)`；`INDEX(status)`；`INDEX(lead_type)` |
| `audit_log` | `INDEX(entity_type, entity_id)`；`INDEX(created_at)`；`INDEX(workflow_run_id)` |
...
### 8.1 主键与时间戳
- 所有表 `id uuid PK DEFAULT gen_random_uuid()`（`image_type_priorities` 除外，以 `image_type` text 为 PK）；所有表 `created_at`，多数含 `updated_at`。
- 时间一律 `timestamptz`；纯日历日（`workflow_batches.week_start/week_end`、`content_metrics.metric_date`）用 `date`。

### 8.2 类型与范围
- 评分列（五维 + `business_relevance`）`smallint CHECK 1-10`；`source_quality_score` `smallint CHECK 1-5`；率值 `numeric(5,4)`；金额 `numeric(12,2)`；计数 `bigint`。
- 所有 `CHECK` 由数据库强制（§7.2），应用层不得依赖"约定优于强制"。

### 8.3 外键完整性
- 关键 FK 一律 `NOT NULL` 指向受控表（如 `source_packets.topic_id`、`content_assets.topic_id`、`content_metrics.topic_id`）。
- 两处跨实体弱引用需应用层校验：`deep_dive_image_plans.source_ref_id`（指向 `brand_assets.id` 或 `source_packet_items.id`，单列无 FK 可施，见 §11 OQ-DM-04）；`topics.source_topic_ids`（uuid[] 数组，FK 语义经 `topic_relations` 边表背书）。

### 8.4 包级核验 rollup（禁止手填）
`source_packets.verification_status` 由明细 `source_packet_items.item_verification_status` 按优先级 rollup 得出，禁止手填与明细不一致。优先级：存在任何 `conflict` 项 → `conflict`；任何 `needs_update` 项 → `needs_update`；全部 `verified` → `verified`；部分 → `partially_verified`；否则 `unverified`。

### 8.5 幂等与留痕
- `workflow_outputs.applied`（boolean，默认 false）标记 Orchestrator 是否已消费；`applied_at` 记录时间；同一产物只消费一次（幂等回写）。
...
- AI 原始产出一律先进 `workflow_outputs`，**仅人工审核通过后提升为 `content_assets`**。
...
- 分配由 Orchestrator 的 ID 分配职责执行（`workflow_tasks.task_type='id_assign'`），回写 `topics.topic_id`；同批 Topic_ID_List 即本周分配的 ID 集合。

### 9.2 `batch_id` 格式
- `{ISO周}-{WORKFLOW_KIND}`，如 `2026W36-AI-WEEKLY`；`WORKFLOW_KIND ∈ {AI-WEEKLY, GITHUB, EVERGREEN, WECHAT, ORCHESTRATOR}`。
...
### 9.3 Topic 血缘建模
- **采用"自引用列 + 规范化边表"混合方案**：`topics.parent_topic_id`（单 FK 自引用）承载"单父衍生树"（一个主 Topic 最多新增 3 个衍生 Topic）；`topics.source_topic_ids`（`uuid[]`）承载"多对多来源血缘"。
...
- 强制机制：`knowledge_derivations` 表 `round_index ∈ 1..3` 且 `UNIQUE(workflow_run_id, round_index)`（数据库兜底）+ Orchestrator 守卫函数 `checkDerivedTopicBudget`（应用层拦截超限并记 `audit_log`，`action='budget_denied'`）。双保险。

### 9.8 人工审核门禁与执行留痕（硬性原则 5/6）
- 所有评分、定级、查重、聚类、CTA 判断、路由、状态迁移均由 Orchestrator 作为 `workflow_type='orchestrator'` 的 run 落库（`workflow_runs`），子 workflow 经 `parent_run_id` 挂父 run，形成可审计调用树。
- 任何人工审核动作与状态变更写 `audit_log`（`actor = 用户标识 或 ai:run-xxx`）。
- `workflow_runs.status = needs_review` 为人工门禁吸收态：人工通过 → `completed`；人工退回 → 同 run 重跑（`attempt_count + 1`）或标记 `needs_revision`；失败可 retry。

### 9.9 查重聚类（Orchestrator 职责落地）
...
- 选 5-8 条；V1 不足 5 条时按实际通过数发布并在批次标注 `low_candidate` 提示人工（触发 `needs_review`）。
...
> 建表顺序建议按上述编号执行，涉及循环引用（topics ↔ source_packets）的两列允许 `ALTER TABLE ADD COLUMN` 后补，或保持 `topics.source_packet_id` 为可空列在建表后添加。M0 阶段按 roadmap D2 全量落库（34 表，不删减），并附种子数据（`ctas`、`workflow_types`、`workflow_templates` v1、`ai_prompt_templates`、`system_settings` 权重、`image_type_priorities`、`workflow_routing_rules` 默认映射）。

---

## 11. Open Questions

本文档悬而未决、需在 D2（Schema 迁移）或对应实现阶段裁决的问题；编号 `OQ-DM-xx`，与 roadmap §5 的 `OQ-xx` 独立但引用其结论。

| 编号 | 问题 | 影响范围 | 推荐方案 |
|---|---|---|---|
| OQ-DM-01 | `content_assets` 索引 `GIN(asset_key)` 的对象是 `text` 非数组，PostgreSQL 的 GIN 对普通 text 无意义（且 `UNIQUE(asset_key)` 的 btree 索引已覆盖等值/前缀查询）。是否为基线笔误，应删除 GIN 还是改为 btree？[REVIEW] | D2 迁移 | 删除 `GIN(asset_key)`，保留 `UNIQUE(asset_key)`；如需 LIKE 查询支持可用 btree `text_pattern_ops` |
| OQ-DM-02 | `content_metrics` 唯一约束的实现形式：基线给出 `UNIQUE(topic_id, asset_id, platform, metric_date, COALESCE(publication_id::text, ''))` 并注明 PG15 可改用 `UNIQUE NULLS NOT DISTINCT`，需在 D2 定案一种（语义等价）。[REVIEW] | D2 迁移 | 采用 PG15 `UNIQUE NULLS NOT DISTINCT`（更直白、免表达式索引）；若 Drizzle 支持度不足则退回 COALESCE 表达式 |
| OQ-DM-03 | `ctas` 受控词表种子全集：基线仅给出 `book_demo` / `download_whitepaper` / `join_community` / `contact_sales` / `follow_account` / `signup_newsletter` 并附"等"字，是否还有追加条目（如 `watch_video` / `subscribe_channel` / `get_quote`）？ | D2 种子数据 | 以基线 6 条为最小集先行落库，追加条目待需求确认 |
| OQ-DM-04 | `deep_dive_image_plans.source_ref_id` 语义上可指向两种实体（`brand_assets.id` 或 `source_packet_items.id`），单列无法施加 FK 约束，数据层如何保证引用有效性？ | D2 迁移、D13 | V1 保持单列 + 应用层校验（按 `source_status` 判断目标表）；如审计要求提升可拆两列（`brand_asset_id` / `source_packet_item_id`） |
| OQ-DM-05 | `system_settings` 无 `created_at` 列，与全局"所有表含 created_at"约定不符（基线未注明例外），是否有意简化？[REVIEW] | D2 迁移 | 若无意，补 `created_at timestamptz NOT NULL DEFAULT now()`；若有意，在 D2 迁移注释中显式声明例外 |
| OQ-DM-06 | `workflow_tasks.task_type` 与 `audit_log.entity_type/action` 为自由 `text`（可扩展），是否需要受控词表约束（`text + CHECK` 或枚举）？基线定位为"text 可扩展"，但自由值会削弱审计查询可靠性。 | D2 迁移、D9 | 保持 `text`（基线定位可扩展），在应用层维护权威取值清单（`_canonical-workflow.md` §5），并建议 D9 增加取值校验 |
| OQ-DM-07 | `knowledge_topic_bank.b2b_relevance` 冗余列（基线注明"由视图暴露，避免双写发散"）：V1 是直接建列并在写入 `topics` 时同步，还是建 SQL 视图不建列？基线 §3.5 保留列定义，需在 D2 明确实现方式。 | D2 迁移、D7 | 推荐按基线保留列但由应用层单写入口同步（写入 topics 时同事务更新），与 topics 保持一致；视图方案留作 M5 重构候选 |
| OQ-DM-08 | `topics ↔ source_packets` 循环引用的事务一致性在 Drizzle ORM 层的实现：先建 Topic → 再建包 → 回填 `topics.source_packet_id` 的三步事务如何封装（表级 `ALTER TABLE` 后补列 vs 可空列先建）？ | D2 迁移 | 推荐 `topics.source_packet_id` 以可空列随建表创建，三步写入包在单一 service 事务内完成（对齐 lineage service 的单写入口模式） |
| OQ-DM-09 | jsonb 动态结构（`key_numbers` / `number_test_conditions` / `event_assessment` / `score_rationale`）的内部 schema 版本管理：字段结构演进时如何兼容存量数据？是否需要 `schema_version` 字段？ | D9、D10 | V1 按基线示例结构固化，不引入内嵌版本号；结构变更走 `audit_log` 留痕 + 迁移文档记录（roadmap OQ-07 内容语言确认后一并定稿示例） |
